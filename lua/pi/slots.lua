...
--- Gap between float *content* boxes. Must be ≥2 so each pane's border sits on
--- its own cells (gap=1 shares one row/col → titles wipe neighbors → looks 错位).
local LAYOUT_GAP = 2
...
--- Mark nbr.{N,S,E,W} for optional styling (borders no longer share cells).
local function annotate_neighbors(out, gap)
  gap = gap or LAYOUT_GAP
  local list = {}
...
--- Independent float frame (gap≥2 so borders never share a cell with neighbors).
---@param slot PiSlot
...
local function refresh_slot_title(slot)
  if slot.win and vim.api.nvim_win_is_valid(slot.win) then
    -- Only title — re-setting border repaints shared gutter neighbors wrongly.
...
function M.apply_layout()
  ...
  -- AC badge sits on the top border row and cuts titles — hide while tiled.
  pcall(function()
    require("pi.compact_indicator").hide()
  end)
  pcall(function()
    require("pi.statusline").repaint()
  end)
